    ...
    <div class="h-full flex flex-col md:flex-row gap-4">
        ...
        <div class="flex-1 flex flex-col items-center justify-center overflow-hidden">
            <div class="w-full max-w-xl aspect-square bg-slate-800 rounded-2xl shadow-lg p-3">
                <ChessBoard :fen="currentFen" :last-move="lastMove" :interactive="false" />
            </div>

            <!-- Controls -->
            <div class="mt-4 flex gap-3">
                <button class="px-3 py-1 rounded-full text-xs font-semibold border border-slate-400/60
                   bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100" @click="handleStepForward"
                    :disabled="!canStep">
                    ▶ Step
                </button>
                <button class="px-3 py-1 rounded-full text-xs font-semibold border border-slate-400/60
                   bg-emerald-500 text-white disabled:bg-slate-400 disabled:text-slate-900" @click="handleTogglePlay"
                    :disabled="!hasOpening">
                    {{ isPlaying ? 'Pause' : 'Play' }}
                </button>
                <button class="px-3 py-1 rounded-full text-xs font-semibold border border-slate-400/60
                   bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100" @click="handleReset"
                    :disabled="!hasOpening">
                    ⟲ Reset
                </button>
            </div>
